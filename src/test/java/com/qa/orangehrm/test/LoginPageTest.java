...
@Feature("US - 501 : create test for login system on HRM using valid and invalid credentials")
...
public class LoginPageTest {
	...
	@Description("verify forget password link method")
	@Severity(SeverityLevel.NORMAL)
	public void verifyForgetPassLink() {
		log.info("starting Forget pass ---------------> login page");
		Assert.assertTrue(loginPage.checkForgetPassLink());
		log.info("ending forget pass ---------------> login page");
		log.warn("some warning");
		log.error("some error");
		log.fatal("fatal error");
	}
	
	@Test(priority = 3, description = "verify login valid credentials", enabled = true)
	@Description("verify login valid credentials")
	...
	public void loginTest() {
		...
		log.info("ending login test ---------------> verify login page");
		log.warn("some warning");
		log.error("some error");
		log.fatal("fatal error");
	}
	...
	@Description("verify login invalid credentials")
